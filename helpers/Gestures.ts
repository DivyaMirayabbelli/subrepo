...
class Gestures {
    ...
    */
    static async clickAtLocation(x: number, y: number) {
        const clickX = x;
        const clickY = y;
     ...
     */
    static async pressElementAndSwipeLeft(elementpath) {
        const { x, y } = await driver.getElementRect(await elementpath.elementId);
        const xStart = await elementpath.getLocation('x');
        const yStart = await elementpath.getLocation('y');
        const width = await elementpath.getSize('width');
        const height = await elementpath.getSize('height');
        const startX = x + (0.8 * width);
        const startY = yStart + (0.5 * height);
        const endX = (SWIPE_DIRECTION.left) ? 0 : 1440;
        const endY = startY;
     ...
     * The given coordinates are percentages of screen.
     */
    static async pressElementAndSwipeRight(elementpath) {
        const location = await elementpath.getLocation();
        const size = await elementpath.getSize();
        const startX = location.x + (size.width * 0.2);
        const endX = location.x + (size.width * 0.8);
        const startY = location.y + size.height * 0.5;
